...
- Memorystore also reuses a verified `cloudrun` connection. `memorystoreAccess="inspect"` stages `roles/redis.viewer`; `memorystoreAccess="lifecycle"` separately stages `roles/redis.admin`. Pub/Sub is independent and is granted only by `queueAccess="lifecycle"`; the standalone `queueAccess="remove"` operation confirmation-gates removal of that exact role from the deploy service account without enabling/disabling APIs, granting other access, or modifying other principals.
