...
Built-in hosting support for Vercel, Render, Heroku, DigitalOcean, and AWS App Runner was removed. Replacement DigitalOcean, Render, Heroku, Azure Container Apps, Vercel, and AWS ECS on Fargate hosting adapters now pass mocked lifecycle and managed exact-SHA/image workflow contracts, but remain behind the provider-conformance live promotion gate and are not advertised as supported yet. Vercel is `ready-for-live` through the review-gated managed-workflow harness documented in [Provider conformance](docs/provider-conformance.md); the other replacement hosting entries remain planned until their live profiles are enabled. The matrix still tracks Fly Machines as a planned target. Supported database providers remain `supabase`, `cloudsql`, `railway`, and `rds`; Azure PostgreSQL, Fly Managed Postgres, Neon, DigitalOcean, and Render are conformance targets at various pre-support stages. Stored connections for removed providers can still be deleted with `hv_connect action="remove"`.
