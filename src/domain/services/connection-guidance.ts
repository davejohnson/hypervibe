...
const GUIDANCE: Record<string, ConnectionGuidance> = {
  ...
  github: {
    ...
    permissions: [
      ...
      'If using a fine-grained PAT for apiToken, grant Metadata read, Contents read/write, Issues read/write, Pull requests read/write, Workflows write, Actions read/write, Secrets write, and Administration write for branch protection on the target repo; add Pages write for GitHub Pages custom domains. GHCR package access still requires a classic PAT (GitHub: "GitHub Packages only supports authentication using a personal access token (classic)"). Hypervibe cannot pre-check fine-grained permissions (no scopes header), so missing permissions surface at apply time.',
    ],
