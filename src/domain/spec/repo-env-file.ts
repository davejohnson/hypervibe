...
import { environmentVariableCoverage } from '../services/environment-variable-coverage.service.js';
...
export interface LocalEnvRequirement {
  ...
  /** A commented, value-free reference, not an input to the dotenv loader. */
...
function normalizedRequirements(requirements: LocalEnvRequirement[]): LocalEnvRequirement[] {
  ...
  for (const requirement of requirements) {
    ...
    if (!comment) continue;
    byKey.set(key, { key, comment, documentationOnly: requirement.documentationOnly });
  }
...
export function ensureCommentedEnvFile(
  ...
  for (const requirement of normalized) {
    const activateEmpty = options.activateEmptyCommentedAssignments && !requirement.documentationOnly;
    ...
    lines.push(comment, `${requirement.documentationOnly ? '# ' : ''}${requirement.key}=`);
...
/**
 * Document spec-declared runtime keys without copying their values. Comments
 * distinguish delegated inputs from generated secrets and spec-owned values.
 * Infrastructure/provider outputs are not inferred or turned into inputs.
 */
export function specLocalEnvRequirements(
  ...
  for (const [key, secret] of Object.entries(spec.secrets)) {
    if (secret.ownership === 'hypervibe') {
      if (selectedEnvironmentName && !secret.environments.includes(selectedEnvironmentName)) continue;
      const destinations = joinDestinations(selectedEnvironmentName ? [selectedEnvironmentName] : secret.environments);
      const comment = `Runtime secret generated by Hypervibe for ${destinations}; keep this entry empty. Local values are not deployment inputs for this key.`;
      requirements.push({ key, comment, templateComment: comment, documentationOnly: true });
      continue;
    }
  ...
  const ordinaryDestinations = new Map<string, string[]>();
  for (const [name, environment] of Object.entries(spec.environments)) {
    if (selectedEnvironmentName && name !== selectedEnvironmentName) continue;
    for (const key of Object.keys(environment.envVars)) {
      ordinaryDestinations.set(key, [...(ordinaryDestinations.get(key) ?? []), name]);
    }
  }
  for (const [key, destinations] of ordinaryDestinations) {
    const comment = `Runtime value declared in the spec for ${joinDestinations(destinations)}; keep this entry empty to use the reviewed spec value.`;
    requirements.push({ key, comment, templateComment: comment, documentationOnly: true });
  }
  if (selectedEnvironmentName) {
    for (const issue of environmentVariableCoverage(spec).issues) {
      if (issue.reason !== 'missing_environment' || issue.environment !== selectedEnvironmentName) continue;
      const comment = `Runtime key declared for related services in ${joinDestinations(issue.declaredIn)}; not managed here. Keep empty; existing live values and ownership are preserved.`;
      requirements.push({ key: issue.key, comment, templateComment: comment, documentationOnly: true });
    }
  }

  // Environment files document scoped runtime keys. Explicit
...
export function ensureRepoEnvTemplate(
  ...
  const templateRequirements = requirements.map((requirement) => ({
    key: requirement.key,
    documentationOnly: requirement.documentationOnly,
