...
describe('repo local env file', () => {
  ...
  it('documents Hypervibe-owned keys with no values or requests for user input', () => {
    ...
    for (const key of ['SESSION_SECRET', 'APP_SIGNING_SECRET']) {
      expect(requirements.find((entry) => entry.key === key)?.comment).toContain('generated by Hypervibe');
      expect(requirements.find((entry) => entry.key === key)?.comment).not.toContain('add the value');
    }
  });

  it.each(['railway', 'cloudrun', 'digitalocean', 'fly', 'ecs', 'azure-container-apps', 'vercel'])(
    'prepares value-free runtime documentation in every environment on %s without adopting production values', (provider) => {
      const root = mkdtempSync(path.join(tmpdir(), 'hypervibe-runtime-docs-'));
      initRepo(root);
      execFileSync('git', ['remote', 'add', 'origin', 'git@github.com:tests/runtime-docs.git'], { cwd: root });
      vi.stubEnv('HYPERVIBE_DISABLE_REPO_SPEC', '0');
      const document = projectSpecSchema.parse({
        version: 1, project: 'runtime-docs',
        secrets: { APP_SIGNING_KEY: { ownership: 'hypervibe', generator: 'random-base64url-32-v1', environments: ['staging'] } },
        environments: {
          production: { hosting: { provider }, services: { web: {} }, envFile: { mode: 'off' } },
          staging: { hosting: { provider }, services: { web: {} }, envFile: { mode: 'off' }, envVars: { CONTACT_EMAIL: 'chosen@example.test' } },
        },
      });
      writeFileSync(path.join(root, '.env.production'), 'CONTACT_EMAIL=existing-production-owner\n', { mode: 0o600 });
      try {
        const result = writeRepoSpecFile(document, root)!;
        expect(result.environmentEnvFiles.map((entry) => path.basename(entry.path))).toEqual(['.env.production', '.env.staging']);
        for (const file of ['.env', '.env.example', '.env.staging', '.env.production']) {
          const content = readFileSync(path.join(root, file), 'utf8');
          expect(content).toMatch(/^# APP_SIGNING_KEY=$/m);
          expect(content).toContain('CONTACT_EMAIL=');
          expect(content).not.toContain('chosen@example.test');
          if (file !== '.env.example') {
            expect(isIgnored(root, file)).toBe(true);
            expect(mode(path.join(root, file))).toBe(0o600);
          }
        }
        expect(readFileSync(path.join(root, '.env.production'), 'utf8')).toContain('CONTACT_EMAIL=existing-production-owner');
        expect(readFileSync(path.join(root, '.env.production'), 'utf8')).toContain('not managed here');
        expect(readFileSync(path.join(root, '.env.staging'), 'utf8')).toContain('generated by Hypervibe');
        const loaded = loadDeployEnvFile({ startDir: root, envName: 'staging', mode: 'all', excludeKeys: ['APP_SIGNING_KEY'] });
        expect(loaded?.vars).toEqual({});
        const before = readFileSync(path.join(root, '.env.production'), 'utf8');
        const repeated = writeRepoSpecFile(document, root)!;
        expect(repeated.environmentEnvFiles.every((entry) => !entry.addedKeys.length && !entry.commentedKeys.length)).toBe(true);
        expect(readFileSync(path.join(root, '.env.production'), 'utf8')).toBe(before);
        // Documentation must not mask a later explicit portable base input.
        writeFileSync(path.join(root, '.env'), 'CONTACT_EMAIL=local-override@example.test\n', { mode: 0o600 });
        expect(loadDeployEnvFile({ startDir: root, envName: 'staging', mode: 'all', excludeKeys: ['APP_SIGNING_KEY'] })?.vars)
          .toEqual({ CONTACT_EMAIL: 'local-override@example.test' });
        expect(JSON.parse(readFileSync(path.join(root, '.hypervibe/spec.json'), 'utf8')).secrets.APP_SIGNING_KEY.environments).toEqual(['staging']);
      } finally {
        vi.unstubAllEnvs();
        rmSync(root, { recursive: true, force: true });
      }
  ...
  it('rejects a tracked environment file before writing any placeholders or spec', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'hypervibe-tracked-runtime-docs-'));
    initRepo(root);
    execFileSync('git', ['remote', 'add', 'origin', 'git@github.com:tests/runtime-docs.git'], { cwd: root });
    vi.stubEnv('HYPERVIBE_DISABLE_REPO_SPEC', '0');
    writeFileSync(path.join(root, '.env'), '# Preserve exactly\n');
    writeFileSync(path.join(root, '.env.production'), '# Tracked file\n');
    execFileSync('git', ['add', '.env.production'], { cwd: root });
    try {
      const document = projectSpecSchema.parse({ version: 1, project: 'runtime-docs', environments: {
        production: { hosting: { provider: 'railway' }, services: { web: {} }, envVars: { CONTACT_EMAIL: 'chosen@example.test' } },
      } });
      expect(() => writeRepoSpecFile(document, root)).toThrow('git already tracks .env.production');
      expect(readFileSync(path.join(root, '.env'), 'utf8')).toBe('# Preserve exactly\n');
      expect(existsSync(path.join(root, '.hypervibe/spec.json'))).toBe(false);
    } finally {
      vi.unstubAllEnvs();
      rmSync(root, { recursive: true, force: true });
    }
