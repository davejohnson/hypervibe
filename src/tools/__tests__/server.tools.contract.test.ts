...
const EXPECTED_TOOLS = [
  ...
  // Domains + email
  'hv_domain_setup', 'hv_domain_register', 'hv_dns_record', 'hv_email_setup', 'hv_email_forwarding', 'hv_email_send',
  // Payments
...
describe('server tool surface', () => {
  ...
  it('every tool responds with the structured envelope on error paths', async () => {
    ...
    const probes: Array<{ name: string; args: Record<string, unknown> }> = [
      ...
      { name: 'hv_domain_register', args: { action: 'check', domain: 'example.com' } },
