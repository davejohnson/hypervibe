...
case "$deploy_sha" in
  *[!0-9a-fA-F]*|'') echo "HYPERVIBE_DEPLOY_SHA must be a full Git SHA" >&2; exit 1 ;;
esac
if [ "\${#deploy_sha}" -ne 40 ]; then
  echo "HYPERVIBE_DEPLOY_SHA must be a full 40-character Git SHA" >&2
  exit 1
fi
expected_sha="\${1:-}"
if [ -z "$expected_sha" ] || [ "$deploy_sha" != "$expected_sha" ]; then
  echo "The checked-out Git SHA does not match the reviewed deploy SHA" >&2
  exit 1
fi
...
image_uri="$CI_REGISTRY_IMAGE:$deploy_sha"
case "$image_uri" in
  *[!A-Za-z0-9._/:@-]*|'') echo "GitLab registry returned an unsafe image URI" >&2; exit 1 ;;
esac
